...
namespace ec
{
    ...
    inline void Measurement::writeScoreFile() const noexcept
    {
        ...
        for (const auto &[key, value] : s_measPoints)
        {
            if (value.m_countSpeed != 0 || value.m_countEnergy != 0 || false)
            {
                ostrm << "  \""
                      << "speed: " << value.m_name << "\" : " << value.m_countSpeed << " * " << value.m_speedWeight << ": " << value.m_countSpeed * value.m_speedWeight << ",\n";
                ostrm << "  \""
                      << "energy: " << value.m_name << "\" : " << value.m_countEnergy << " * " << value.m_energyWeight << ": " << value.m_countEnergy * value.m_energyWeight << ",\n";
            }
        }
    ...
    inline void Measurement::inc(Measurement::MeasType measType) noexcept
    {
        ...
        if (isFromVecHw(measType))
            addUsageOfVecHw();
        if (isFromStreamHw(measType))
            addUsageOfStreamHw();
